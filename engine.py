...
from models.dino.gatector import get_gt_gaze_cone
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets, target_gaze, face, head_channel, gaze_heatmap in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        for b in target_gaze:
            head_box = b['head_box'].to(device)
            head_boxes.append(head_box)

        target_gaze_point = [g['gaze_point'].data.numpy() for g in target_gaze]
        eye_position = [g['eye'].data.numpy() for g in target_gaze]
        gt_gaze_cone_size = 15
        gt_gaze_cone = get_gt_gaze_cone(eye_position, target_gaze_point, gt_gaze_cone_size)
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            gt_gaze_cone = torch.from_numpy(gt_gaze_cone).to(device)
            gaze_cone_loss = mse_loss(gaze_cone, gt_gaze_cone)
            loss_dict.update({'loss_gaze_cone': gaze_cone_loss})
            weight_dict.update({'loss_gaze_cone': 1})
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    with open(args.output_dir+'score.csv', 'a', encoding='utf8', newline='') as f:
        writer = csv.writer(f)
