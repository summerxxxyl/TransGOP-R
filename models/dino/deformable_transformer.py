...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        od_memory = memory.detach().clone()
        od_pos = lvl_pos_embed_flatten.detach().clone()
        od_mask = mask_flatten.detach().clone()
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, od_memory, od_pos, od_mask
        # hs: (n_dec, bs, nq, d_model)
