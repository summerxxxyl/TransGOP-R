...
def select_head(outputs, target_sizes, not_to_xyxy=False, test=False):
    num_select = 100
    ...
    # head_label_id = labels == 25
    #
    # labels = labels[head_label_id]
    # scores = scores[head_label_id]
    # boxes = out_bbox[head_label_id]
...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, gatector, num_classes, num_queries,
    ...
    def forward(self, samples: NestedTensor,face, head_channel, targets:List=None, head_boxes:List=None):
        ...
        gatector_scene_feature = features[2].tensors
        gatector_head_feature = features[2].tensors
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, od_memory, od_pos, od_mask = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask)
        ...
        # orig_target_sizes = torch.stack([targets["orig_size"].cuda()], dim=0)

        heads = select_head(out, orig_target_sizes)
        if head_boxes is None:
            head_boxes = []
        gaze_outputs, gaze_cone = self.gatector(gatector_scene_feature, gatector_head_feature, heads, head_boxes, orig_target_sizes, od_memory, od_pos, od_mask)



        return out, gaze_outputs, gaze_cone
