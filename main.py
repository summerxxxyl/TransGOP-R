...
def get_args_parser():
    ...
    parser.add_argument('--coco_path', type=str, default='/data1/gcx002/Datasets/gooreal/R-TransGOP/')
...
def main(args):
    ...
    if args.resume:
        ...
        if args.use_ema:
            if 'ema_model' in checkpoint:
                ...
                del ema_m
                ema_m = ModelEma(model, args.ema_decay)                
        # use epoch ,lr and optimizer of checkpoint #
    ...
    if os.path.exists(args.output_dir+'score.csv'):
        headers = ['auc', 'dist', 'ang']
        with open('logs/score.csv', 'a', encoding='utf8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(headers)
    else:
        headers = ['auc', 'dist', 'ang']
        with open(args.output_dir+'score.csv', 'w+', encoding='utf8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(headers)
    ...
    # for param in model.gatector.parameters():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, Freeze_data_loader_train, optimizer, device, epoch,
