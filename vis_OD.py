...
if __name__ == '__main__':
    ...
    model_checkpoint_path = "logs_synth/checkpoint0026.pth"
    img_dir = "/data1/gcx002/Datasets/goosynth10000/gaze_dino/val/"
    img_save_dir = "vis_OD_synth/"
    ann = "/data1/gcx002/Datasets/goosynth10000/gaze_dino/annotations/gop_val.json"

    # model_checkpoint_path = "logs_256_pretrain_real/best_gaze.pth"  # change the path of the model checkpoint
    # See our Model Zoo section in README.md for more details about our pretrained models.
    # img_dir = "/data1/gcx002/Datasets/gooreal/goo_to_coco/val2017"
    # ann = "/data1/gcx002/Datasets/gooreal/gaze-dino/annotations/instances_val2017.json"

    with open('goo_name.json') as f:
        id2name = json.load(f)
    ...
    # image = Image.open('/data1/gcx002/Datasets/gooreal/goo_to_coco/val2017/3616.jpg').convert("RGB")
        ...
        output, gaze_output, gaze_cone = model.cuda()(img[None].cuda(), face, head_channel, target)
        output = postprocessors['bbox'](output, torch.Tensor([[1.0, 1.0]]).cuda())[0]

        scores = output['scores']
        labels = output['labels']
        boxes = output['boxes']
        select_mask = scores > 0.5
        ...
        for index in range(len(boxes)):
            p_box = boxes[index].clone()
            p_box = p_box * torch.tensor([width, height, width, height]).cuda()

            ovmax = 0

            for g_box in gt_box:

                n_x = (min(p_box[2], g_box[2]) - max(p_box[0], g_box[0]) + 1)
                n_y = (min(p_box[3], g_box[3]) - max(p_box[1], g_box[1]) + 1)
                n = n_x * n_y
                u = ((p_box[2] - p_box[0] + 1) * (p_box[3] - p_box[1] + 1) + (g_box[2] - g_box[0] + 1) * (
                            g_box[3] - g_box[1] + 1))
                if u > n and n_x > 0 and n_y > 0:
                    iou = n / (u - n)
        ...
        vslzr.visualize(img, pred_dict, savedir=img_save_dir,
